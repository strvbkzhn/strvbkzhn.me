...
<svelte:head>
	<title>Bekzhan Satarov</title>
	<meta name="description" content="Bekzhan Satarov's personal website" />
	<script src="https://challenges.cloudflare.com/turnstile/v0/api.js" defer></script>
</svelte:head>

<main class="flex-1">
	<section class="mx-auto max-w-5xl px-4 py-8">
		<p class="max-w-prose flex-2 text-justify text-balance">
			Hey there,
			<br />
			<br />
			I'm Bekzhan. I'm a frontend web developer with a bit of backend know-how.
			<br />
			<br />
			Currently, I'm a part of the <Button href="https://www.thetradingpit.com/" external
				>The Trading Pit</Button
			>, enhancing the user experience for prop traders. Before that, I led the development team at <Button
				href="https://wedevx.co/"
				external>Wedevx</Button
			>, an all-in-one learning platform. I earned my BSc in Computer Science at <Button
				href="https://inai.kg/en/"
				external>INAI.KG</Button
			>, with a year at <Button href="https://www.fh-zwickau.de/english/" external>WHZ</Button> during
			the program.
			<br />
			<br />
			When I'm not coding, I either enjoying football or exploring new tech trends and working on personal
			projects.
			<br />
			<br />
		</p>
		{#if form?.success === true}
			<span class="text-green-500">Message received — thanks for connecting!</span>
		...
		{:else}
			Let's connect! —
			<Dialog title="Leave a message">
				{#snippet dialogTrigger(open)}
					<Button as="button" onclick={open}>[Leave me a message]</Button>
				{/snippet}
				<form method="post" class="mt-2 flex w-full flex-col gap-3">
					<textarea
						name="message"
						id="message"
						...
						class="peer w-full resize-none overflow-hidden rounded-xs border-2 border-dotted border-blue-200 bg-blue-50/30 px-3.5 py-3 outline-none placeholder:text-neutral-400"
					></textarea>
					<div
						class="cf-turnstile peer/turnstile"
						data-sitekey={PUBLIC_CLOUDFLARE_SITE_KEY}
						data-size="flexible"
						data-theme="light"
					></div>
					<div
						class="font-cloudflare -mt-3 hidden w-full items-center justify-between gap-4 border border-[#E0E0E0] bg-[#FAFAFA] px-4 py-3 peer-empty/turnstile:flex"
					>
						<div class="flex items-center gap-3">
							<svg viewBox="0 0 30 30" aria-hidden="true" class="h-[30px] w-[30px]">
								<circle class="expired-circle" cx="15" cy="15" r="15" fill="#999999"></circle>
								<path
									fill="#FFFFFF"
							...
							<div>
								<p class="text-[14px] leading-[16px] font-normal text-balance">Error</p>
								<p class="text-[9px] leading-[12px] font-medium">
									<span>A page</span>
									<button
										type="button"
										onclick={() => window?.location?.reload()}
										class="cursor-pointer appearance-none underline">refresh</button
									>
									<span>is required to send a message.</span>
								</p>
							</div>
						</div>
						<div class="flex flex-col items-end gap-[4px]">
							<a
								...
								></a
							>
							<div class="text-[8px] leading-[10px] font-normal">
								<a
									id="privacy-link"
									target="_blank"
									rel="noopener noreferrer"
					...
					<Button as="button" wrapperClass="self-end">Send</Button>
				</form>
			</Dialog>
		{/if}
	</section>
